...
def calc_tsne(dataMatrix,NO_DIMS=2,PERPLEX=30,INITIAL_DIMS=30,LANDMARKS=1):
    ...
    """
    if dataMatrix.shape[1] > 1500:
        print "Too high, random choice 1500"
        idx = arange(dataMatrix.shape[1])
        random.shuffle(idx)
        dataMatrix = dataMatrix[:,idx[0:1000]]
...
def PCA(dataMatrix, INITIAL_DIMS) :
    ...
    print 'Minus the datamean'
    dataMatrix= dataMatrix-dataMatrix.mean(axis=0)
    
    print 'Judge the Feature Dim greater than INITIAL_DIMS?'
    if dataMatrix.shape[1]<INITIAL_DIMS:
        INITIAL_DIMS=dataMatrix.shape[1]

    print 'Compute the eigValues and eigVectors'
